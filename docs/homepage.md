...
**Card display**
- Each card leads with a **verdict thumbnail** (`src/components/VerdictThumb.astro`,
  `variant="card"`) — an inline tinted block keyed to the verdict colour tokens,
  showing the verdict word + neutral `fcheck.in` wordmark. It replaces the old
  emoji placeholder and is used by both `TrendingCard.astro` and `ReportCard.astro`.
  A null verdict (TYPE 4) renders an honest "Under review" state, not a guess.
- Below the thumbnail: verdict badge (color-coded), claim headline, source badge
  (fcheck.in vs. external fact-checker name), date
- Attribution is always visible; the source badge, not the thumbnail, carries the
  fcheck.in-vs-external distinction (the thumbnail brand line stays neutral so it
  never mis-credits an external checker's verdict)
...
The article page (`article/[slug].astro`) uses the same component with
`variant="hero"` as a banner above the headline.
...
### Social preview images (og:image) *(built — static per-verdict)*

Both share surfaces emit an `og:image` (+ `twitter:card=summary_large_image`) so a
link dropped into WhatsApp/X/Telegram previews as a branded card instead of bare
text. `Base.astro` takes an optional `image` prop, pointed at a static 1200×630 PNG
chosen by verdict:

- `article/[slug].astro` (TYPE 1 originals) → `public/og/verdict-<verdict>.png`
- `check/[id].astro` → same, but **only for shareable published verdicts** (TYPE
  1/2). TYPE 3 (provisional) and TYPE 4 (submitted) get no card — there is no
  settled verdict to show.

**Two branding variants**, because the verdict's owner differs by type:
- `verdict-<v>.png` — "✓ Verified by fcheck.in" — used for **originals** (our verdict).
- `verdict-<v>-ext.png` — neutral "✓ fcheck.in" wordmark — used for **external**
  (TYPE 2). The verdict belongs to the attributed fact-checker, so the card must
  not claim fcheck.in verified it (the credit-external rule). Attribution to the
  specific source stays on the page.

These are **committed static assets** (13 PNGs: 6 verdicts × 2 variants +
`under-review.png`), regenerated by `npm run gen:og` (`scripts/gen-og.mjs`,
headless Chrome; palette mirrors `tokens.css` — keep in sync).

Because the set is per-verdict, the headline is **not** on the share image yet. A
per-article rendered PNG (headline + verdict) is a possible fast-follow — see
[roadmap.md](roadmap.md).

> **WhatsApp caches link previews.** A link shared *before* its `og:image`
> existed keeps showing the old image-less preview for a while; WhatsApp re-fetches
> only periodically (a cache-busting query param forces a fresh crawl when testing).
