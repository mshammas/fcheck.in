...
Article pages emit an `og:image` (+ `twitter:card=summary_large_image`) so a link
dropped into WhatsApp/X/Telegram previews as a branded card instead of bare text.
`Base.astro` takes an optional `image` prop; `article/[slug].astro` points it at a
static 1200×630 PNG chosen by verdict — `public/og/verdict-<verdict>.png` (and
`under-review.png`). These are **committed static assets**, regenerated by
`npm run gen:og` (`scripts/gen-og.mjs`, headless Chrome; palette mirrors
`tokens.css` — keep in sync). The article route is originals-only, so the
"Verified by fcheck.in" branding on these cards is always accurate.
...
[roadmap.md](roadmap.md). External `/check/[id]` pages are not wired to og:image
(a fcheck.in-branded card would mis-credit the external checker).
