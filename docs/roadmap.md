...
- **Built:** `VerdictThumb.astro` (inline tinted, verdict-keyed) replaced the emoji
  ...
  band. Article pages also emit a per-verdict `og:image` from committed static PNGs
  in `public/og/` (regenerated by `npm run gen:og` → `scripts/gen-og.mjs`). See
...
- **Fast-follow (not built):** a per-article rendered share PNG (headline + verdict +
  branding), so each shared link gets a unique card. Needs raster generation on the
  Worker (satori + resvg-wasm) or generate-at-publish into R2. The static per-verdict
  set is the interim; `og:image` on external `/check/[id]` pages is intentionally left
  off to avoid mis-crediting external checkers.
